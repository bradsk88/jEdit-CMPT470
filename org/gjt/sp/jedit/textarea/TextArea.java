 ...
 * concurrent manipulation of multiple, non-contiguous regions of text. Methods
 * in this class that deal with selecting text rely upon classes derived the
 * {@link Selection} class.
 * 
 * @author Slava Pestov
...
public class TextArea extends JComponent {
	...
	public TextArea(InputHandlerProvider inputHandlerProvider) {
		...
		add(ScrollLayout.BOTTOM, horizontal = new JScrollBar(Adjustable.HORIZONTAL));
	...
	// {{{ getGutter() method
	/**
	 * Returns the gutter to the left of the text area or null if the gutter is
	 * disabled
	 */
	...
	/**
	 * @return the minimum distance (in number of lines) from the caret to the
	 *         nearest edge of the screen (top or bottom edge).
	 */
	...
	/**
	 * Returns if clicking the middle mouse button pastes the most recent
	 * selection (% register), and if Control-dragging inserts the selection at
	 * the caret.
	 */
	...
	/**
	 * Sets if clicking the middle mouse button pastes the most recent selection
	 * (% register), and if Control-dragging inserts the selection at the caret.
	 * 
	 * @param quickCopy
	 *            A boolean flag
	 */
	 ...
	 * @since jedit 4.3pre3
	 * 
	 *        Prior to 4.3pre3, this function returned a "Buffer" type. If this
	 *        causes your code to break, try calling view.getBuffer() instead of
	...
	// {{{ setBuffer() method
	/**
	 * Sets the buffer this text area is editing. Do not call this method - use
	 * {@link org.gjt.sp.jedit.EditPane#setBuffer(Buffer)} instead.
	 * 
	 * @param buffer
	 *            The buffer
	 */
	 ...
	 * necessary support to call a hook method via reflection. This method is
	 * called by the {@link org.gjt.sp.jedit.Java14} class to signal that a drag
	 * is in progress.
	 * 
	 * @since jEdit 4.2pre5
	 */
	 ...
	 * 
	 * @param physFirstLine
	 *            The first physical line to display
	 * @since jEdit 4.2pre1
	 */
	 ...
	 * necessary.
	 * 
	 * @param doElectricScroll
	 *            If true, electric scrolling will be performed
	 */
	...
	public void scrollTo(int line, int offset, boolean doElectricScroll) {
		...
		if (point == null)
		// FIXME - we need to reset the state of this window so that it has the
		// right
		// dimensions again.
		{

			Log.log(Log.ERROR, this, "BUG: screenLine=" + screenLine + ",visibleLines=" + visibleLines
					+ ",physicalLine=" + line + ",offset=" + offset + ",firstPhysicalLine=" + getFirstPhysicalLine()
	...
	// {{{ xyToOffset() method
	/**
	 * Converts a point to an offset. Note that unlike in previous jEdit
	 * versions, this method now returns -1 if the y co-ordinate is out of
	 * bounds.
	 * 
	 * @param x
	...
	/**
	 * Converts a point in a given screen line to an offset. Note that unlike in
	 * previous jEdit versions, this method now returns -1 if the y co-ordinate
	 * is out of bounds.
	 * 
	 * @param x
	 *            The x co-ordinate of the point
	...
	// {{{ offsetToXY() method
	/**
	 * Converts an offset into a point in the text area painter's co-ordinate
	 * space.
	 * 
	 * @param offset
	 *            The offset
	 * @return The location of the offset on screen, or <code>null</code> if the
	 *         specified offset is not visible
	 */
	 ...
	 * 
	 * @param line
	 *            The physical line number (from top of document)
	 * @param offset
	 *            The offset in characters, from the start of the line
	 * @param retVal
	 *            The point to store the return value in
	 * @return <code>retVal</code> for convenience, or <code>null</code> if the
	 *         specified offset is not visible
	 * @since jEdit 4.0pre4
	 */
	 ...
	 * 
	 * @param line
	 *            The physical line to invalidate
	 */
	public void invalidateLine(int line) {
		if (!isShowing() || buffer.isLoading() || line < getFirstPhysicalLine() || line > physLastLine
	...
	/**
	 * Selects from the bracket at the specified position to the corresponding
	 * bracket.
	 * 
	 * @since jEdit 4.2pre1
	 */
	...
	/**
	 * Returns if the specified line is contained in the currently matched
	 * structure's scope.
	 * 
	 * @since jEdit 4.2pre3
	 */
	...
	/**
	 * Returns the number of selections. This can be used to test for the
	 * existence of selections.
	 * 
	 * @since jEdit 3.2pre2
	 */
	...
	// {{{ getSelection() method
	/**
	 * Returns the selection with the specified index. This must be between 0
	 * and the return value of <code>getSelectionCount()</code>.
	 * 
	 * @since jEdit 4.3pre1
	 * @param index
	 *            the index of the selection you want
	 */
	...
	// {{{ setSelection() method
	/**
	 * Sets the selection. Nested and overlapping selections are merged where
	 * possible. Null elements of the array are ignored.
	 * 
	 * @param selection
	 *            The new selection since jEdit 3.2pre1
	 */
	...
	/**
	 * Adds to the selection. Nested and overlapping selections are merged where
	 * possible.
	 * 
	 * @param selection
	 *            The new selection since jEdit 3.2pre1
	 ...
	 * <code>null</code> if there is no selection at that offset.
	 * 
	 * @param offset
	 *            The offset
	 * @since jEdit 3.2pre1
	...
	// {{{ removeFromSelection() method
	/**
	 * Deactivates the selection at the specified offset. If there is no
	 * selection at that offset, does nothing.
	 * 
	 * @param offset
	 *            The offset
	 * @since jEdit 3.2pre1
	...
	/**
	 * Resizes the selection at the specified offset, or creates a new one if
	 * there is no selection at the specified offset. This is a utility method
	 * that is mainly useful in the mouse event handler because it handles the
	 * case of end being before offset gracefully (unlike the rest of the
	 * selection API).
	 * 
	 * @param offset
	 *            The offset
	 ...
	 * @param extraEndVirt
	 *            Only for rectangular selections - specifies how far it extends
	 *            into virtual space.
	 * @param rect
	 *            Make the selection rectangular?
	 * @since jEdit 3.2pre1
	 */
	...
	public void disableWhitespace() {
		painter.disableWhitespaceMarkers();
		painter.setEOLMarkersPainted(false);
	}

	public void enableWhitespace() {
		painter.enableWhitespaceMarkers();
		painter.setEOLMarkersPainted(true);
	}

	// {{{ extendSelection() method
	/**
	 * Extends the selection at the specified offset, or creates a new one if
	 * there is no selection at the specified offset. This is different from
	 * resizing in that the new chunk is added to the selection in question,
	 ...
	 * @param end
	 *            The new selection end
	 * @param extraStartVirt
	 *            Extra virtual space at the start
	 * @param extraEndVirt
	 *            Extra virtual space at the end
	 * @since jEdit 4.2pre1
	 */
	 ...
	 * 
	 * @param separator
	 *            The string to insert between each text chunk (for example, a
	 *            newline)
	 * @since jEdit 3.2pre1
	 */
	...
	// {{{ getSelectedText() method
	/**
	 * Returns the text in all active selections, with a newline between each
	 * text chunk.
	 */
	...
	// {{{ setSelectedText() method
	/**
	 * Replaces the selection at the caret with the specified text. If there is
	 * no selection at the caret, the text is inserted at the caret position.
	 */
	 ...
	 *            The new selection
	 * @param moveCaret
	 *            Move caret to insertion location if necessary
	 * @since jEdit 4.2pre5
	 */
	 ...
	 * in a buffer. The line numbers in the array returned by this method can be
	 * passed as a parameter to such methods as
	...
	/**
	 * Returns the structure element (bracket, or XML tag, etc) matching the one
	 * before the caret.
	 * 
	 * @since jEdit 4.2pre3
	 */
	...
	/**
	 * Returns an internal position used to keep the caret in one column while
	 * moving around lines of varying lengths.
	 * 
	 * @since jEdit 4.2pre1
	 */
	...
	public void goToNextLine(boolean select) {
		...
		int newCaret = chunkCache.getBelowPosition(caretLine, caret - buffer.getLineStartOffset(caretLine), magic + 1,
				rectSelect && select);
		if (newCaret == -1) {
			int end = getLineEndOffset(caretLine) - 1;
			if (caret == end) {
	...
	// {{{ goToNextWord() method
	/**
	 * Moves the caret to the start of the next word. Note that if the
	 * "view.eatWhitespace" boolean propery is false, this method moves the
	 * caret to the end of the current word instead.
	 * 
	 * @param select
	 *            true if you want to extend selection
	 * @since jEdit 2.7pre2
	...
	public void goToPrevLine(boolean select) {
		...
		int newCaret = chunkCache.getAbovePosition(caretLine, caret - buffer.getLineStartOffset(caretLine), magic + 1,
				rectSelect && select);
		if (newCaret == -1) {
			int start = getLineStartOffset(caretLine);
			if (caret == start) {
	 ...
	 * non-whitespace character of the line, then the beginning of the line,
	 * then to the first visible line.
	 * 
	 * @param select
	 *            true if you want to extend selection
	 * @since jEdit 4.3pre7
	 ...
	 * non-whitespace character of the line, then the end of the line, then to
	 ...
	 * <li>Inserting a space with automatic abbrev expansion enabled will try to
	 * expand the abbrev
	 ...
	 * <code>multi</code>. This only affects the ability to make multiple
	 * selections in the user interface; macros and plugins can manipulate them
	 * regardless of the setting of this flag. In fact, in most cases, calling
	 * this method should not be necessary.
	 * 
	 * @param multi
	 *            Should multiple selection be enabled?
	 * @since jEdit 3.2pre1
	 */
	 ...
	 * <code>rectangularSelectionMode</code>. This only affects the ability to
	 * make multiple selections from the keyboard. A rectangular selection can
	 * always be created by dragging with the mouse by holding down
	 * <b>Control</b>, regardless of the state of this flag.
	 * 
	 * @param rectangularSelectionMode
	 *            Should rectangular selection be enabled?
	 * @since jEdit 4.2pre1
	 */
	...
	// {{{ collapseFold() method
	/**
	 * Like {@link DisplayManager#collapseFold(int)}, but also moves the caret
	 * to the first line of the fold.
	 * 
	 * @since jEdit 4.0pre3
	 */
	...
	public void collapseFold(int line) {
		...
		if (!multi) {
			// cannot use selectNone() beacause the finishCaretUpdate method
			// will reopen the fold
			invalidateSelectedLines();
	...
	// {{{ expandFold() method
	/**
	 * Like {@link DisplayManager#expandFold(int,boolean)}, but also moves the
	 * caret to the first sub-fold.
	 * 
	 * @since jEdit 4.0pre3
	 */
	...
	public void addExplicitFold() throws TextAreaException {
		...
		try {
			...
			if (getSelectionCount() == 0) {
				...
				// Selection cannot be null because there is at least 1
				// selection
	...
	public void lineComment() {
		...
		try {
			for (int i = 0; i < lines.length; i++) {
				...
				buffer.insert(getLineStartOffset(lines[i]) + StandardUtilities.getLeadingWhiteSpace(text), comment);
			}
		} finally {
			buffer.endCompoundEdit();
		}
	...
	public void rangeComment() {
		...
		if (!buffer.isEditable() || commentStart == null || commentEnd == null || commentStart.length() == 0
				|| commentEnd.length() == 0) {
			getToolkit().beep();
			return;
		}
	...
	// {{{ joinLines() method
	/**
	 * Join the lines in the selection. If you use this method you have to lock
	 * the buffer in compound edit mode
	 * 
	 * @param selection
	 *            the selection
	 * @since jEdit 4.3pre8
	 ...
	 * ErrorList plugin uses this to show a global error overview, for example.
	 * 
	 * @param comp
	 *            The component
	 * @since jEdit 4.2pre1
	...
	// {{{ addNotify() method
	/**
	 * Called by the AWT when this component is added to a parent. Adds document
	 * listener.
	 */
	@Override
	 ...
	 * clears the pointer to the currently focused component. Also removes
	 * document listener.
	 */
	@Override
	...
	/**
	 * Removes a component from above the gutter, text area, and vertical scroll
	 * bar.
	 * 
	 * @since jEdit 4.2pre3
	 */
	...
	public void propertiesChanged() {
		...
		charWidth = (int) Math.round(painter.getFont().getStringBounds(foo, 0, 1, painter.getFontRenderContext())
				.getWidth());
	...
	/**
	 * @deprecated Instead, obtain a Selection instance using any means, and
	 *             call its <code>getStart()</code> method
	 */
	@Deprecated
	...
	/**
	 * @deprecated Instead, obtain a Selection instance using any means, and
	 *             call its <code>getStart(int)</code> method
	 */
	@Deprecated
	...
	/**
	 * @deprecated Instead, obtain a Selection instance using any means, and
	 *             call its <code>getStartLine()</code> method
	 */
	@Deprecated
	...
	/**
	 * @deprecated Instead, obtain a Selection instance using any means, and
	 *             call its <code>getEnd()</code> method
	 */
	@Deprecated
	...
	/**
	 * @deprecated Instead, obtain a Selection instance using any means, and
	 *             call its <code>getEnd(int)</code> method
	 */
	@Deprecated
	...
	/**
	 * @deprecated Instead, obtain a Selection instance using any means, and
	 *             call its <code>getEndLine()</code> method
	 */
	@Deprecated
	...
	// {{{ select() method
	/**
	 * @deprecated Instead, call either <code>addToSelection()</code>, or
	 *             <code>setSelection()</code> with a new Selection instance.
	 */
	@Deprecated
	...
	/**
	 * @deprecated Instead, check if the appropriate Selection is an instance of
	 *             the Selection.Rect class.
	 */
	@Deprecated
	...
	final boolean isStructureHighlightVisible() {
		return match != null && hasFocus() && displayManager.isLineVisible(match.startLine)
	...
	void updateMaxHorizontalScrollWidth() {
		...
		if (max != maxHorizontalScrollWidth) {
			...
			horizontal
					.setValues(Math.max(0,
							Math.min(maxHorizontalScrollWidth + charWidth - painter.getWidth(), -horizontalOffset)),
							painter.getWidth(), 0, maxHorizontalScrollWidth + charWidth);
	 ...
	 * of lines in the buffer changes, or when the size of the text are changes.
	 */
	...
	/**
	 * the collapsing of scrolling/event firing inside compound edits greatly
	 * speeds up replace-all.
	 */
	...
	protected void userInputTab() {
		if (getSelectionCount() == 1) {
			...
			if (sel instanceof Selection.Rect
					|| (sel.startLine == sel.endLine && (sel.start != buffer.getLineStartOffset(sel.startLine) || sel.end != buffer
							.getLineEndOffset(sel.startLine) - 1))) {
				insertTab();
			} else
				shiftIndentRight();
	...
	protected boolean doWordWrap(boolean spaceInserted) {
		...
		/*
		 * only ever return true if space was pressed with logicalLength ==
		 * maxLineLen
		 */
		return returnValue;
	} // }}}
	...
	private void updateStructureHighlight() {
		...
		if (match != null) {
			...
			if (!displayManager.isLineVisible(match.startLine)
					|| chunkCache.getScreenLineOfOffset(match.startLine, match.start
							- getLineStartOffset(match.startLine)) == -1) {
	...
	private RectParams getRectParams(int caret, int newCaret) {
		...
		if (s instanceof Selection.Rect) {
			if (caret == s.end) {
				virtualWidth = buffer.getVirtualWidth(s.endLine, s.end - getLineStartOffset(s.endLine))
						+ ((Selection.Rect) s).extraEndVirt;
			} else {
				virtualWidth = buffer.getVirtualWidth(s.startLine, s.start - getLineStartOffset(s.startLine))
						+ ((Selection.Rect) s).extraStartVirt;
			}
	...
	private void shiftTallCaretLeft(Selection.Rect s) {
		removeFromSelection(s);
		addToSelection(new Selection.Rect(buffer, s.getStartLine(), s.getStartColumn(buffer) - 1, s.getEndLine(),
				s.getEndColumn(buffer) - 1));
	} // }}}
	...
	private void setMaxLineLength(int maxLineLen) {
		...
		if (maxLineLen <= 0) {
			...
			wrapToWidth = false;
			wrapMargin = (int) painter.getFont().getStringBounds(foo, 0, foo.length, painter.getFontRenderContext())
					.getWidth();
		}
	} // }}}
	...
	// {{{ addExplicitFold() method
	/**
	 * Add an explicit fold. You should call this method inside a compoundEdit
	 * in the buffer. You must also check if the buffer fold mode is explicit
