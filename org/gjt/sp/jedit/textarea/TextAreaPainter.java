 ...
 * 
 * @author Slava Pestov
 * @version $Id: TextAreaPainter.java 7156 2006-10-02 21:33:17Z kpouer $
 */
public class TextAreaPainter extends JComponent implements TabExpander {
	...
	public void paint(Graphics _gfx) {
		...
		if (height == 0 || buffer.isLoading()) {
			...
			FastRepaintManager.RepaintLines lines = textArea.repaintMgr.prepareGraphics(clipRect,
			...
			extensionMgr.paintScreenLineRange(textArea, gfx, lines.first, lines.last, y, height);
			...
			if (Debug.PAINT_TIMER && numLines >= 1)
				Log.log(Log.DEBUG, this, "repainting " + numLines + " lines took " + prepareTime + "/" + linesTime
						+ "/" + blitTime + " ms");
		}
	...
	private final PaintText extension;
	...
	TextAreaPainter(TextArea textArea) {
		...
		extension = new PaintText(ShowMarks.WHITESPACE);
		addExtension(TEXT_LAYER, extension);
	...
	class PaintLineBackground extends TextAreaExtension {
		...
		public void paintValidLine(Graphics2D gfx, int screenLine, int physicalLine, int start, int end, int y) {
			...
			boolean collapsedFold = (physicalLine < buffer.getLineCount() - 1 && buffer.isFoldStart(physicalLine) && !textArea.displayManager
	...
	class PaintText extends TextAreaExtension {
		...
		private ShowMarks whitespace;

		public PaintText(ShowMarks whitespace) {
		...
		public void paintValidLine(Graphics2D gfx, int screenLine, int physicalLine, int start, int end, int y) {
			...
			if (lineInfo.chunks != null) {
				x += Chunk.paintChunkList(lineInfo.chunks, gfx, textArea.getHorizontalOffset(), baseLine,
						!Debug.DISABLE_GLYPH_VECTOR, whitespace);
			}
		...
		public void enableWhitespace() {
			this.whitespace = ShowMarks.WHITESPACE;
		}

		public void disableWhitespace() {
			this.whitespace = ShowMarks.NONE;
		}
	} // }}}
	...
	class PaintCaret extends TextAreaExtension {
		...
		public void paintValidLine(Graphics2D gfx, int screenLine, int physicalLine, int start, int end, int y) {
			...
			if (textArea.isOverwriteEnabled()) {
				gfx.drawLine(caretX, y + height - 1, caretX + textArea.charWidth, y + height - 1);
			} else if (blockCaret)
	...
	public void enableWhitespaceMarkers() {
		extension.enableWhitespace();
	}

	public void disableWhitespaceMarkers() {
		extension.disableWhitespace();
	}
